...
interface StoredSession { input: SessionInput; revokedAt: Date | null; replacedBySessionId: string | null }
...
class MemoryAuthRepository extends AuthRepository {
  ...
  createUser(email: string, passwordHash: string, displayName: string | null): Promise<AuthUserRecord> {
    const user = { id: `user-${this.users.size + 1}`, email, passwordHash, displayName, deletedAt: null };
  ...
  async rotateSession(oldSessionId: string, oldTokenHash: string, next: SessionInput, now: Date): Promise<RotationResult> {
    const current = this.sessions.get(oldSessionId);
    const user = current === undefined ? undefined : this.users.get(current.input.userId);
    if (current === undefined || user === undefined || current.input.tokenHash !== oldTokenHash || current.input.expiresAt <= now) return { status: "invalid" };
    if (current.revokedAt !== null || current.replacedBySessionId !== null) {
      for (const session of this.sessions.values()) if (session.input.familyId === current.input.familyId) session.revokedAt ??= now;
  ...
  revokeSession(tokenHash: string, now: Date): Promise<void> {
    for (const session of this.sessions.values()) if (session.input.tokenHash === tokenHash) session.revokedAt ??= now;
    return Promise.resolve();
  }
}
...
function fixture(): AuthService {
  const values: AppEnvironment = {
    NODE_ENV: RuntimeEnvironment.Test, API_PORT: 3001,
    DATABASE_URL: "postgresql://user:password@localhost:5432/test", REDIS_URL: "redis://localhost:6379",
    JWT_ACCESS_SECRET: "a".repeat(64), JWT_REFRESH_SECRET: "b".repeat(64), JWT_ACCESS_TTL: "15m", JWT_REFRESH_TTL: "30d",
    S3_ENDPOINT: "http://localhost:9000", S3_ACCESS_KEY: "test", S3_SECRET_KEY: "test-secret", S3_BUCKET: "test-bucket", S3_REGION: "test-region",
    WEB_URL: "http://localhost:3000", API_URL: "http://localhost:3001", INTERNAL_API_URL: "http://localhost:3001", COLLAB_URL: "ws://localhost:3002",
    REVALIDATION_SECRET: "c".repeat(64),
  };
  return new AuthService(new MemoryAuthRepository(), new JwtService(), new ConfigService<AppEnvironment, true>(values));
}

const registration = { email: "Person@Example.com", password: "StrongPassword123", displayName: "Person" };
...
test("login issues a new persisted session", async () => {
  const auth = fixture(); await auth.register(registration, {});
  const result = await auth.login({ email: registration.email, password: registration.password }, {});
  assert.equal(result.user.email, "person@example.com");
});

test("login rejects an invalid password", async () => {
  const auth = fixture(); await auth.register(registration, {});
  await assert.rejects(auth.login({ email: registration.email, password: "wrong" }, {}), UnauthorizedException);
});

test("refresh rotates the persisted session", async () => {
  const auth = fixture(); const initial = await auth.register(registration, {});
