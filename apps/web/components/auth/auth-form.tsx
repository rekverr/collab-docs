  ...
  return (
    <main className="auth-page">
      <section className="auth-card" aria-labelledby="auth-title">
        ...
        <p className="muted">
          {isRegister
            ? "Organize knowledge with your team."
        ...
        <form onSubmit={submit} className="form-stack">
          {isRegister && (
            <label>
              Display name <input name="displayName" maxLength={120} autoComplete="name" />
            </label>
          )}
          <label>
            Email <input name="email" type="email" required maxLength={320} autoComplete="email" />
          </label>
          <label>
            Password{" "}
            <input
              ...
              pattern={isRegister ? "(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9]).{12,128}" : undefined}
            />
          </label>
          {isRegister && (
            <p className="field-help">
              12–128 characters with upper-case, lower-case, and a number.
            </p>
          )}
          {error !== null && (
        ...
        <p className="auth-switch">
          {isRegister ? "Already have an account?" : "New to Collab Docs?"}{" "}
