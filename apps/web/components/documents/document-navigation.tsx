...
export function DocumentNavigation({
  ...
  role,
  selectedDocumentId,
}: Readonly<DocumentNavigationProps>) {
  ...
  useEffect(() => {
    ...
    setError(null);
    void session
      .withAccessToken((token) => documentApi.tree(token, workspaceId))
      .then((result) => {
  ...
  async function create(parentId?: string): Promise<void> {
    const title = window.prompt(
      parentId === undefined ? "Root document title" : "Child document title",
      "Untitled",
    );
    ...
    setError(null);
    try {
      const created = await session.withAccessToken((token) =>
        documentApi.create(token, workspaceId, {
          title: title.trim(),
  ...
  async function rename(node: DocumentTreeNode): Promise<void> {
    ...
    const previous = tree;
    setTree(renameDocument(tree, node.id, title.trim()));
    setBusy(true);
    setError(null);
    try {
  ...
  async function remove(node: DocumentTreeNode, action: "archive" | "delete"): Promise<void> {
    if (
      !window.confirm(`${action === "archive" ? "Archive" : "Delete"} “${node.title}”?`) ||
      tree === null
    )
      return;
    const previous = tree;
    setTree(removeDocument(tree, node.id));
    setBusy(true);
    setError(null);
    try {
      await session.withAccessToken((token) =>
        action === "archive"
          ? documentApi.archive(token, node.id)
          : documentApi.delete(token, node.id),
      );
      await loadTree();
      if (
        selectedDocumentId === node.id ||
        (selectedDocumentId !== undefined && containsDocument(node.children, selectedDocumentId))
      ) {
  ...
  async function drop(
    ...
    if (optimistic === null) {
      setError("That move would create an invalid document hierarchy.");
      return;
    }
    const previous = tree;
    setTree(optimistic);
    ...
    try {
      await session.withAccessToken((token) =>
        documentApi.move(token, documentId, {
          parentId,
          ...(beforeDocumentId === undefined ? {} : { beforeDocumentId }),
        }),
      );
      await loadTree();
  ...
  return (
    <aside className="document-navigation" aria-label="Document navigation">
      <div className="document-navigation-header">
      ...
      {tree?.length === 0 && (
        <div className="document-empty">
          ...
          {canEdit && (
            <button className="text-button" type="button" onClick={() => void create()}>
      ...
      <div role="tree" aria-busy={busy}>
        {tree?.map((node) => (
          <TreeNode
            ...
            onOpen={(id) => router.push(`/app/workspaces/${workspaceId}/documents/${id}`)}
            onCreate={create}
            onRename={rename}
            onRemove={remove}
...
interface TreeNodeProps {
  ...
  onRename(node: DocumentTreeNode): Promise<void>;
  onRemove(node: DocumentTreeNode, action: "archive" | "delete"): Promise<void>;
  onDragStart(id: string | null): void;
  onDrop(documentId: string, parentId: string | null, beforeDocumentId?: string): Promise<void>;
}
...
function TreeNode(props: Readonly<TreeNodeProps>) {
  ...
  function allowDrop(event: DragEvent<HTMLElement>): void {
    if (canEdit && draggedId !== null && !busy) event.preventDefault();
  }
  return (
      ...
      {canEdit && (
        <div
          ...
          onDrop={(event) => {
            ...
            event.stopPropagation();
            if (draggedId !== null) void props.onDrop(draggedId, node.parentId, node.id);
          }}
          aria-hidden="true"
        />
      )}
      <div
        className={`document-row${selectedDocumentId === node.id ? " selected" : ""}`}
        style={{ paddingLeft: `${depth * 1.05 + 0.35}rem` }}
        ...
        onDrop={(event) => {
          event.preventDefault();
          event.stopPropagation();
          if (draggedId !== null) void props.onDrop(draggedId, node.id);
        }}
      >
        <button
        ...
        <button className="document-open" type="button" onClick={() => props.onOpen(node.id)}>
          {node.title}
        </button>
        {canEdit && (
            ...
            {canDelete && (
              <>
                <button
                  ...
                  onClick={(event) => {
                    stop(event);
                    void props.onRemove(node, "archive");
                  }}
                >
                  ⌁
                </button>
                <button
                  ...
                  onClick={(event) => {
                    stop(event);
                    void props.onRemove(node, "delete");
                  }}
                >
                  ×
                </button>
