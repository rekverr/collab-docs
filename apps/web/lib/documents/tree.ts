...
export function containsDocument(nodes: readonly DocumentTreeNode[], documentId: string): boolean {
  return nodes.some(
    (node) => node.id === documentId || containsDocument(node.children, documentId),
  );
}
...
export function removeDocument(
  ...
  return nodes
    ...
    .map((node) => ({ ...node, children: removeDocument(node.children, documentId) }));
}
...
function detachDocument(
  ...
  if (index >= 0)
    return { node: nodes[index]!, tree: [...nodes.slice(0, index), ...nodes.slice(index + 1)] };
  for (const node of nodes) {
    const detached = detachDocument(node.children, documentId);
    if (detached !== null) {
      return {
        node: detached.node,
        tree: nodes.map((candidate) =>
          candidate.id === node.id ? { ...candidate, children: detached.tree } : candidate,
        ),
      };
    }
  }
...
function insertDocument(
  ...
  if (parentId === null) {
    const index =
      beforeId === undefined ? nodes.length : nodes.findIndex(({ id }) => id === beforeId);
    ...
    if (candidate.id === parentId) {
      const index =
        beforeId === undefined
          ...
          : candidate.children.findIndex(({ id }) => id === beforeId);
...
export function moveDocumentOptimistically(
  ...
  if (documentId === parentId || (parentId !== null && isDescendantOf(nodes, parentId, documentId)))
    return null;
