...
import type { CollaborationPersistence, CompactionResult, PersistedDocumentState, PersistUpdateInput, StoredUpdate } from "./persistence.js";
import { DocumentUnavailableError } from "./persistence.js";

export interface PrismaPersistenceOptions { compactAfterUpdates?: number; retainedSnapshots?: number }
...
export class PrismaCollaborationPersistence implements CollaborationPersistence {
  ...
  load(documentId: string): Promise<PersistedDocumentState> {
    return this.prisma.$transaction(async (transaction) => {
      ...
      const snapshot = await transaction.yjsSnapshot.findFirst({
        where: { documentId }, orderBy: { sequence: "desc" }, select: { sequence: true, state: true },
      });
      const updates = await transaction.yjsUpdate.findMany({
        where: { documentId, sequence: { gt: snapshot?.sequence ?? -1n } }, orderBy: { sequence: "asc" }, select: { sequence: true, update: true },
      });
      return {
        snapshot: snapshot === null ? null : { sequence: snapshot.sequence, state: bytes(snapshot.state) },
        updates: updates.map((update) => ({ sequence: update.sequence, update: bytes(update.update) })),
      };
  ...
  async storeUpdate(input: PersistUpdateInput): Promise<StoredUpdate> {
    ...
    const result = await this.prisma.$transaction(async (transaction) => {
      ...
      const existing = await transaction.yjsUpdate.findUnique({
        where: { documentId_updateHash: { documentId: input.documentId, updateHash } }, select: { sequence: true },
      });
      if (existing !== null) return { sequence: existing.sequence, duplicate: true, published: document.publicationState === "PUBLISHED" };
      const maximum = await transaction.yjsUpdate.aggregate({ where: { documentId: input.documentId }, _max: { sequence: true } });
      const latestSnapshot = await transaction.yjsSnapshot.findFirst({
        where: { documentId: input.documentId }, orderBy: { sequence: "desc" }, select: { sequence: true },
      });
      const sequence = maxBigInt(maximum._max.sequence ?? 0n, latestSnapshot?.sequence ?? 0n) + 1n;
      await transaction.yjsUpdate.create({
        data: { documentId: input.documentId, actorUserId: input.actorUserId, sequence, updateHash, update: Buffer.from(input.update) },
      });
      await transaction.document.update({
        where: { id: input.documentId },
        data: { contentProjection: projection, projectionSequence: sequence, projectionUpdatedAt: new Date() },
      });
      return { sequence, duplicate: false, published: document.publicationState === "PUBLISHED" };
    }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });

    await this.publisher.publish({
      documentId: input.documentId, sequence: result.sequence, projection: input.projection, published: result.published,
    });
    if (!result.duplicate && result.sequence % BigInt(this.compactAfterUpdates) === 0n) await this.compact(input.documentId);
  ...
  compact(documentId: string): Promise<CompactionResult> {
    ...
    void compaction.then(() => this.localCompactions.delete(documentId), () => this.localCompactions.delete(documentId));
    return compaction;
  }
  ...
  private async performCompaction(documentId: string): Promise<CompactionResult> {
    const durable = await this.prisma.$transaction(async (transaction) => {
      ...
      const latest = await transaction.yjsSnapshot.findFirst({
        where: { documentId }, orderBy: { sequence: "desc" }, select: { id: true, sequence: true, state: true },
      });
      const updates = await transaction.yjsUpdate.findMany({
        where: { documentId, sequence: { gt: latest?.sequence ?? -1n } }, orderBy: { sequence: "asc" }, select: { sequence: true, update: true },
      });
      if (updates.length === 0) return latest === null ? null : { id: latest.id, sequence: latest.sequence, created: false };
      ...
      const snapshot = await transaction.yjsSnapshot.create({
        data: {
          documentId, sequence, state: Buffer.from(state), stateVector: Buffer.from(Y.encodeStateVector(document)), contentHash: hash(state),
        },
    ...
    if (!durable.created) return { compacted: false, sequence: durable.sequence, removedUpdates: 0 };

    const removedUpdates = await this.prisma.$transaction(async (transaction) => {
      await lockDocument(transaction, documentId);
      const snapshot = await transaction.yjsSnapshot.findUnique({ where: { id: durable.id }, select: { id: true } });
      ...
      const deleted = await transaction.yjsUpdate.deleteMany({ where: { documentId, compactedBySnapshotId: durable.id } });
      const retained = await transaction.yjsSnapshot.findMany({
        where: { documentId }, orderBy: { sequence: "desc" }, take: this.retainedSnapshots, select: { id: true },
      });
      await transaction.yjsSnapshot.deleteMany({ where: { documentId, id: { notIn: retained.map(({ id }) => id) } } });
...
async function requireActiveDocument(transaction: Transaction, documentId: string) {
  const document = await transaction.document.findFirst({
    where: { id: documentId, deletedAt: null, archivedAt: null }, select: { id: true, publicationState: true },
  });
  if (document === null) throw new DocumentUnavailableError();
  return document;
}

function bytes(value: Uint8Array): Uint8Array { return new Uint8Array(value.buffer, value.byteOffset, value.byteLength); }
