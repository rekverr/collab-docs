...
const viewer: CollaborationIdentity = { documentId, userId: "viewer", email: "viewer@example.com", displayName: "Viewer", canWrite: false };
...
class FakeAuthorizer implements CollaborationAuthorizer {
  ...
  authorize(token: string, requestedDocumentId: string): Promise<CollaborationIdentity> {
    ...
    if (identity === undefined || identity.documentId !== requestedDocumentId) return Promise.reject(new AuthorizationFailure("permission", "denied"));
...
async function start(identities: Readonly<Record<string, CollaborationIdentity>>): Promise<{ server: CollaborationServer; url: string }> {
  const server = new CollaborationServer(
    { port: 0, host: "127.0.0.1", authenticationTimeoutMs: 1000, authorizationRecheckMs: 0 },
    new FakeAuthorizer(identities), new MemoryPersistence(), silentLogger,
  );
  servers.push(server);
...
describe("authenticated Yjs collaboration", () => {
  it("accepts an authorized connection", async () => {
    ...
    const initial = onceBinaryMessage(socket); authenticate(socket, "valid"); await initial;
  ...
  it("rejects an unauthorized connection before room join", async () => {
    ...
    const socket = await connect(url);
    const closed = onceClose(socket); authenticate(socket, "invalid"); const code = await closed;
  ...
  it("rejects a Viewer Yjs write", async () => {
    ...
    const socket = await connect(url);
    const initial = onceBinaryMessage(socket); authenticate(socket, "viewer"); await initial;
  ...
  it("places two clients in one room and synchronizes a Yjs update", async () => {
    const { server, url } = await start({ first: editor, second: { ...editor, userId: "editor-2", email: "second@example.com" } });
    const first = await connect(url);
    const second = await connect(url);
    const firstInitial = onceBinaryMessage(first); const secondInitial = onceBinaryMessage(second);
    authenticate(first, "first"); authenticate(second, "second");
...
function syncUpdateMessage(update: Uint8Array): Uint8Array {
  const encoder = encoding.createEncoder(); encoding.writeVarUint(encoder, 0); syncProtocol.writeUpdate(encoder, update);
...
function onceBinaryMessage(socket: WebSocket): Promise<Uint8Array> {
  return new Promise((resolve, reject) => {
    socket.once("message", (data, isBinary) => isBinary ? resolve(bytes(data)) : reject(new Error("Expected binary message")));
