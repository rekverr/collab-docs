...
const compactAfterUpdates = parsePositiveInteger(process.env.CRDT_COMPACT_AFTER_UPDATES ?? "100", "CRDT_COMPACT_AFTER_UPDATES");
...
function requireRedisUrl(value: string): string {
  ...
  if (url.protocol !== "redis:" && url.protocol !== "rediss:") throw new Error("REDIS_URL must use Redis protocol");
  return url.toString();
}
